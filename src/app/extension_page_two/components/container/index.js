...
import Info from '../../../extension_page_one/components/info/index.js';
...
export default class Container extends Component{
  ...
  updateValue=(text,type,isTrue,errorText)=>{
	...
	//弹窗按钮1点击事件 暂不认证
  btn1Click=()=>{
    window.location.href='../extension_pagefail_two/extension_pagefail_two.html?success=false';
  }
      ...
      if(strUtil.isEmpty(this.state.imgCodeErrorText)){
        ...
        if(strUtil.isEmpty(this.state.phoneCodeErrorText)){
          ...
          if(strUtil.isEmpty(this.state.psdErrorText)){
              ...
              var param;
              if(dept == '') {
                 url=`${config.baseUrl}/user/register/registerUser.json`;
                 ...
                 url=`${config.baseUrl}/user/register/registerH5User.json`;
              ...
              let successCall=(data)=>{
                ...
                window.location.href='../extension_pageSucess_two/extension_pageSucess_two.html?success=false';
              }
              let errorCall=(data)=>{
                this.setState({isLoading: false});
    ...
    var button=dealBtn();
		...
		let getPhone=()=>{
			if(this.state.phoneBtnDisabled){
				phone=<PhoneCode className='phone-code disabled' disabled
          ...
          successUrl='../extension_pagefail_two/extension_pagefail_two.html?success=true'/>
			}else{
